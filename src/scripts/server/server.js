...
if (config_1.args.login || config_1.args.admin) {
    ...
    // 添加原始assets目录支持，解决404问题
    app.use('/assets', express.static(paths_1.pathTo('assets'), { maxAge, etag }));
...
if (config_1.args.game) {
    ...
    theWorld = world;
    // 更新当前游戏服务器状态为在线
    const currentServer = internal_1.findServer(config_1.server.id);
    if (currentServer) {
        currentServer.state.offline = false;
        currentServer.state.dead = false;
        currentServer.state.online = 1;
    }
